...
public class ApiBackend extends ApiBase {
    ...
    public void postConstruct() {
        ...
        for (Map.Entry<String, ResourceMatchersConfig.Matcher> entry : map.entrySet()) {
            ...
            requestMatchers.add(createPathMatcher(pattern, method));
            logger.info("Config ResourceMatchers: {} '{}'", method, pattern);
        }
        // Mandatory, not an empty RequestMatcher!
        if (requestMatchers.isEmpty()) {
            requestMatchers.add(createPathMatcher("/**", null));
    ...
    private RequestMatcher createPathMatcher(String pattern, String method) {
        String cleanPattern = pattern;
        if (cleanPattern != null && cleanPattern.startsWith("*/**")) {
            cleanPattern = "/**";
        } else if (cleanPattern != null && !cleanPattern.startsWith("/")) {
            cleanPattern = "/" + cleanPattern;
        }

        PathPattern pathPattern = PathPatternParser.defaultInstance.parse(cleanPattern != null ? cleanPattern : "/**");

        return request -> {
            if (method != null && !method.trim().isEmpty() && !method.equalsIgnoreCase(request.getMethod())) {
                return false;
            }
            // Safely extract the path within the application
            String pathWithinApp = request.getServletPath();
            if (request.getPathInfo() != null) {
                pathWithinApp += request.getPathInfo();
            }
            return pathPattern.matches(PathContainer.parsePath(pathWithinApp.isEmpty() ? "/" : pathWithinApp));
        };
    }
    ...
    public final Object requestEntity(@RequestBody(required = false) String body, HttpMethod method,
        ...
        try {
            ...
            if (entityBackend.getBody() instanceof Resource resource) {
